...
const word_array = (text) =>
    clean_string(text)              // ALUMNA
        .match(/[a-zñáéíóú]+/gi);   // ALUMNA
...
const word_index = () => {
    ...
    let ordered_words = word_array(text)    // ALUMNA
        .map((el) => el.toLowerCase())
        .sort();

    let result = repetitions(ordered_words) // ALUMNA
        .map((el) => `${el.s}: ${el.n}`)
        .join("\n");
...
const search_words = () => {
    ...
    let result = `Hay ${ordered_words.length} ocurrencias de la palabra o letra '${search}'.\n\n`; // ALUMNA

    result += repetitions(ordered_words)    // ALUMNA
        .map((el) => `${el.n} repeticiones de:  ${el.s}`)
        .join("\n");
